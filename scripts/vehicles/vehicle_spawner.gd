...
func _despawn_far() -> void:
	...
	var remove: Array[VehicleBase] = []
	for v in _active:
		if not is_instance_valid(v): continue
		if v.driver != null: continue  # Never despawn occupied
		var near := false
		for pp in positions:
		...
		if not near:
			v.queue_free()
			remove.append(v)
	for r in remove: _active.erase(r)

func _spawn_batch() -> void:
	var density: float = GameManager.rules.get("traffic_density", 1.0)
	if density <= 0.0: return
	var cap := int(MAX_VEHICLES_PER_PLAYER * GameManager._players.size() * density)
	...
	for _i in range(2):
		...
		var pp := positions[randi() % positions.size()]
		var angle := randf() * TAU
...
func _spawn_at(pos: Vector3) -> void:
	...
	var v: VehicleBase = _scenes[type].instantiate()
	v.global_position = pos
	get_tree().root.add_child(v)
	_active.append(v)

func _spawn_placeholder(pos: Vector3) -> void:
	# Box stand-in until real vehicle scenes exist
...
func despawn_all() -> void:
	for v in _active:
		if is_instance_valid(v) and v.driver == null: v.queue_free()
	_active.clear()
